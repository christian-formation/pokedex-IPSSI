...
const PokemonList =()=> {
  ...
  //     },[]);
    ...
    const {pokedex,loading}:context = useContext(MainContext)
    ...
    return (
        <div>
          ...
          <div className="bg-white">
              ...
              {!loading?pokedex.map((pokemon:Object,index:number) => <PokemonItem key={index} slug={pokemon.name} name={pokemon.name} imageAlt={pokemon.name} imageSrc={split(pokemon.url)}></PokemonItem>):""}
              </div>
            </div>
          </div>
        </div>
